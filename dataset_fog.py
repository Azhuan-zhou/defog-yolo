...
class dataset_fog(Dataset):
    def __init__(self, dataset_type):
        ...
        # 多尺度
        self.multiscale = True if dataset_type == 'train' else False
        # train or test
    ...
    def parse_annotation(self, annotation):
        ...
        if random.randint(0, 2) > 0:
            ...
            if self.data_train_flag:  # 如果时训练时
                img_name = cfg.TRAIN.vocfog_traindata_dir + image_name \
                           + '_' + ("%.2f" % beta) + '.' + image_name_index
            else:
                img_name = cfg.TEST.vocfog_valdata_dir + image_name \
                           + '_' + ("%.2f" % beta) + '.' + image_name_index
...
class infer_dataset(dataset_fog):
    def __init__(self, annot_path, size):
        ...
        self.annotations = np.array(self.load_annotations('infer'))
        # 样本个数
        self.num_samples = len(self.annotations)
...
if __name__ == '__main__':
    ...
    for paths, images, targets in fog_loader:
        ...
        class_names = [fog.classes[int(i)] for i in class_names]
        bboxs = targets[:, 2:]
