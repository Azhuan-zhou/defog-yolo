...
def _evaluate(model, dataloader, device, class_names, img_size, iou_thres, conf_thres, nms_thres, verbose):
    ...
    for loader in tqdm.tqdm(dataloader, desc="Validating"):
        # 图片和标签做成变量
        images = loader[0].to(device)

        targets = loader[7].to(device)
        labels += targets[:, 1].tolist()
        targets[:, 2:] = xywh2xyxy(targets[:, 2:])

        with torch.no_grad():
            p, p_decoder = model(images)
            outputs = non_max_suppression(p_decoder, conf_thres=conf_thres, iou_thres=nms_thres)
