...
def save_image(image_tensor, out_x,a,b):
    image_tensor = (image_tensor * 255).squeeze(0).permute(1, 2, 0).detach().numpy()
    out_x = (out_x * 255).squeeze(0).permute(1, 2, 0).detach().numpy()
    arr = np.hstack((image_tensor, out_x))
    cv2.imwrite("check/processed/img-contrast.png", arr)
...
if __name__ == '__main__':
    ...
    print(model)
    out_x, a = model(image_tensor)
    print('output shape:', out_x.shape)
    b = ['wb', 'gamma', 'defog', 'sharpen', 'contrast', 'tone']
    save_image(image_tensor, out_x, a,b)
