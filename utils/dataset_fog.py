import os
import cv2
import random
import numpy as np
import torch
from torch.utils.data import Dataset, DataLoader
from utils.util import to_tensor, get_anchors, read_class_names
from config import cfg
from utils.augmentation import (
    random_horizontal_flip_fog,
    random_crop_fog,
    random_translate_fog,
    random_horizontal_flip,
    random_crop,
    random_translate,
    image_preprocess,
    Mixup
)


def bbox_iou(boxes1, boxes2):
    boxes1 = np.array(boxes1)
    boxes2 = np.array(boxes2)
    # box面积
    boxes1_area = boxes1[..., 2] * boxes1[..., 3]
    boxes2_area = boxes2[..., 2] * boxes2[..., 3]

    boxes1 = np.concatenate([boxes1[..., :2] - boxes1[..., 2:] * 0.5,
                             boxes1[..., :2] + boxes1[..., 2:] * 0.5], axis=-1)
    boxes2 = np.concatenate([boxes2[..., :2] - boxes2[..., 2:] * 0.5,
                             boxes2[..., :2] + boxes2[..., 2:] * 0.5], axis=-1)

    left_up = np.maximum(boxes1[..., :2], boxes2[..., :2])
    right_down = np.minimum(boxes1[..., 2:], boxes2[..., 2:])

    inter_section = np.maximum(right_down - left_up, 0.0)
    inter_area = inter_section[..., 0] * inter_section[..., 1]
    union_area = boxes1_area + boxes2_area - inter_area

    return inter_area / union_area


class dataset_fog(Dataset):
    def __init__(self, dataset_type):
        """
        返回
         image(fog/clean):图片 (bs,3,416,416)
         label_small_bbox: 小尺度检测框 (bs,52,52,3,x+y+w+h+conf+classes) , x,y,w,h均是原图片尺寸的目标位置
         label_middle_bbox: 中尺度检测框 (bs,26,26,3,x+y+w+h+conf+classes)
         label_large_bbox: 大尺度检测框 (bs,13,13,3,x+y+w+h+conf+classes)
         small_bboxes: 小尺度特征图的gt (150,x+y+w+h), x,y,w,h均是原图片尺寸的目标位置
         middle_bboxes: 中尺度特征图的gt (150,4)
         large_bboxes：大尺度特征图的gt (150,4)
         target: 检测框 (num,batch+class+x+y+w+h)
         clean_image: 图片 (bs,416,416,3)

        :param dataset_type: 训练集或测试集？
        """
        super(Dataset, self).__init__()
        self.train_output_sizes = None
        self.train_input_size = None
        self.annot_path = cfg.TRAIN.ANNOT_PATH if dataset_type == 'train' else cfg.TEST.ANNOT_PATH
        # 输入图片尺寸
        self.input_sizes = cfg.TRAIN.INPUT_SIZE if dataset_type == 'train' else cfg.TEST.INPUT_SIZE
        # 一个批次的图片个数
        self.batch_size = cfg.TRAIN.BATCH_SIZE if dataset_type == 'train' else cfg.TEST.BATCH_SIZE
        # 图片增强
        self.data_aug = cfg.TRAIN.DATA_AUG if dataset_type == 'train' else cfg.TEST.DATA_AUG
        # train or test
        self.data_train_flag = True if dataset_type == 'train' else False
        # 训练集输入图片的尺寸
        self.train_input_sizes = cfg.TRAIN.INPUT_SIZE
        # yolo 每次下采样的倍数
        self.strides = np.array(cfg.YOLO.STRIDES)
        # 数据集中目标的名称
        self.classes = read_class_names(cfg.YOLO.CLASSES)
        # 数据集中目标的个数
        self.num_classes = len(self.classes)
        # 锚框（3，3，2）
        self.anchors = np.array(get_anchors(cfg.YOLO.ANCHORS))
        # yolo中个检测层的锚框个数
        self.anchor_per_scale = cfg.YOLO.ANCHOR_PER_SCALE  # 3
        # 一个检测层中最多的box
        self.max_bbox_per_scale = 150
        self.train_input_size = 416  # random.choice(self.train_input_sizes)
        self.train_output_sizes = self.train_input_size // self.strides
        # 图片路径和图片中目标的annotation
        self.annotations = np.array(self.load_annotations(dataset_type))[0:10]
        # 样本个数
        self.num_samples = len(self.annotations)

    # only use the image including the labeled instance objects for training
    def load_annotations(self, dataset_type):
        with open(self.annot_path, 'r') as f:
            txt = f.readlines()
            annotations = [line.strip() for line in txt if len(line.strip().split()[1:]) != 0]  # 有标注的图片
        # np.random.shuffle(annotations)
        print('###################the total image:', len(annotations))
        return annotations

    def __getitem__(self, index):
        image, bboxes, _ = self.parse_annotation(self.annotations[index])
        image = image.permute(2, 0, 1)

        label_small_bbox, label_middle_bbox, label_large_bbox, small_bboxes, middle_bboxes, large_bboxes, target = \
            self.preprocess_true_boxes(bboxes)
        return image, \
               label_small_bbox, label_middle_bbox, label_large_bbox, \
               small_bboxes, middle_bboxes, large_bboxes, \
               target

    def collate_fn(self, batch):
        image, *labels, targets = zip(*batch)
        tensor_targets = []
        for i, target in enumerate(targets):
            target[:, 0] = i
            tensor_targets.append(target)
        return torch.stack(image), *map(torch.stack, labels), torch.cat(tensor_targets, dim=0)

    def parse_annotation(self, annotation):
        """
        对输入的图片和标注主力
        :param annotation: （图片路径，标注）
        :return: image()
        """
        line = annotation.split()
        image_path = line[0]
        if not os.path.exists(image_path):
            raise KeyError("%s does not exist ... " % image_path)
        image = cv2.imread(image_path)
        img_name = image_path.split('/')[-1]  # 图片名字.jpg
        # print(img_name)
        image_name = img_name.split('.')[0]  # 图片名字
        # print(image_name)
        image_name_index = img_name.split('.')[1]  # 图片格式
        # print('*****************read image***************************')
        # bounding box (num_gt,5) 获得标注
        bboxes = np.array([list(map(lambda x: int(float(x)), box.split(','))) for box in line[1:]])
        # 2/3的概率获得带雾图片
        if random.randint(0, 2) > 0:
            beta = random.randint(0, 9)
            beta = 0.01 * beta + 0.05
            if self.data_train_flag:  # 如果时训练时
                img_name = cfg.vocfog_traindata_dir + image_name \
                           + '_' + ("%.2f" % beta) + '.' + image_name_index
            else:
                img_name = cfg.vocfog_valdata_dir + image_name \
                           + '_' + ("%.2f" % beta) + '.' + image_name_index
            foggy_image = cv2.imread(img_name)
            # 是否对图片进行增强
            if self.data_aug:
                # 1/2 的概率水平反转
                image, foggy_image, bboxes = random_horizontal_flip_fog(image, foggy_image, bboxes)
                # 1/2的概率对图像随机裁剪
                image, foggy_image, bboxes = random_crop_fog(image, foggy_image, bboxes)
                # 1/2的概率进行图像平移
                image, foggy_image, bboxes = random_translate_fog(image, foggy_image, bboxes)
            foggy_image, _ = image_preprocess(np.copy(foggy_image),
                                              [self.train_input_size, self.train_input_size],
                                              np.copy(bboxes))
            clean_image, bboxes = image_preprocess(np.copy(image),
                                                   [self.train_input_size, self.train_input_size],
                                                   np.copy(bboxes))
        # 不使用雾天的图片
        else:
            if self.data_aug:
                image, bboxes = random_horizontal_flip(np.copy(image), np.copy(bboxes))
                image, bboxes = random_crop(np.copy(image), np.copy(bboxes))
                image, bboxes = random_translate(np.copy(image), np.copy(bboxes))
            clean_image, bboxes = image_preprocess(np.copy(image),
                                                   [self.train_input_size, self.train_input_size],
                                                   np.copy(bboxes))
            foggy_image = clean_image

        return to_tensor(foggy_image), bboxes, to_tensor(clean_image)

    def preprocess_true_boxes(self, bboxes):
        # 对bbox预处理
        label = [np.zeros((self.train_output_sizes[i], self.train_output_sizes[i], self.anchor_per_scale,
                           5 + self.num_classes)) for i in range(3)]  # 以416为例子，三个不同尺度下的标签 (52，52，3，5+mix+classes)
        # (26,26,3，5+mix+classes) (13,13,3，5+mix+classes)

        target = []
        bboxes_xywh = [np.zeros((self.max_bbox_per_scale, 4)) for _ in range(3)]  # （3, 150，4）
        bbox_count = np.zeros((3,))  # （3，）

        for bbox in bboxes:  # 对一个图片中的每一个gt
            bbox_coor = bbox[:4]  # 左上角和右下角坐标 (4,)
            bbox_class_ind = int(bbox[4])  # 标签

            # 独热编码（软机制）
            onehot = np.zeros(self.num_classes, dtype=np.float64)
            onehot[bbox_class_ind] = 1.0
            uniform_distribution = np.full(self.num_classes, 1.0 / self.num_classes)
            deta = 0.01
            smooth_onehot = onehot * (1 - deta) + deta * uniform_distribution

            # x0 = ( x1 + x2 ) / 2 ; y = ( y1 + y2 ) / 2  ; w = ( x2 - x1 ) / 2 ; h = ( y2 - y1 ) / 2
            bbox_xywh = np.concatenate([(bbox_coor[2:] + bbox_coor[:2]) * 0.5, bbox_coor[2:] - bbox_coor[:2]], axis=-1)
            bbox_xywh_scaled = 1.0 * bbox_xywh[np.newaxis, :] / self.strides[:, np.newaxis]  # (3,4),将gt的尺寸缩放到三个尺度下

            target_b = torch.zeros(1, 6)
            target_b[0, 2:] = torch.tensor(bbox_xywh, dtype=torch.float32)
            target_b[0, 1] = torch.tensor(bbox[4], dtype=torch.float32)
            target.append(target_b)

            iou = []
            exist_positive = False
            for i in range(3):  # 从小目标到大目标
                # 计算anchor 与 ground truth的IOU，只有大于0.3的才作为标签，负责检测目标
                anchors_xywh = np.zeros((self.anchor_per_scale, 4))  # (3,4) # 三个anchor
                anchors_xywh[:, 0:2] = np.floor(bbox_xywh_scaled[i, 0:2]).astype(np.int32) + 0.5  # 网格中心点坐标
                anchors_xywh[:, 2:4] = self.anchors[i]  # （3，2），从小目标到大目标 52，26，13

                iou_scale = bbox_iou(bbox_xywh_scaled[i][np.newaxis, :], anchors_xywh)  # 计算anchor和gt的IOU
                iou.append(iou_scale)
                iou_mask = iou_scale > 0.3

                if np.any(iou_mask):  # 如果三个尺度的anchor中存在IOU>0.3
                    xind, yind = np.floor(bbox_xywh_scaled[i, 0:2]).astype(np.int32)  # 网格位置

                    label[i][yind, xind, iou_mask, 0:4] = bbox_xywh  # 没有缩放过的gt
                    label[i][yind, xind, iou_mask, 4:5] = 1.0
                    label[i][yind, xind, iou_mask, 5:] = smooth_onehot

                    bbox_ind = int(bbox_count[i] % self.max_bbox_per_scale)  # 在0到150之间
                    bboxes_xywh[i][bbox_ind, :4] = bbox_xywh  # 没有缩放过的gt
                    bbox_count[i] += 1  # 记录大中小尺寸下，

                    exist_positive = True
            # 如果没有anchor,选多个尺度下最大的
            if not exist_positive:
                best_anchor_ind = np.argmax(np.array(iou).reshape(-1), axis=-1)  # 最大IOU的anchor
                best_detect = int(best_anchor_ind / self.anchor_per_scale)
                best_anchor = int(best_anchor_ind % self.anchor_per_scale)

                xind, yind = np.floor(bbox_xywh_scaled[best_detect, 0:2]).astype(np.int32)

                label[best_detect][yind, xind, best_anchor, 0:4] = bbox_xywh
                label[best_detect][yind, xind, best_anchor, 4:5] = 1.0
                label[best_detect][yind, xind, best_anchor, 5:] = smooth_onehot

                bbox_ind = int(bbox_count[best_detect] % self.max_bbox_per_scale)
                bboxes_xywh[best_detect][bbox_ind, :4] = bbox_xywh
                bbox_count[best_detect] += 1

        label_sbbox, label_mbbox, label_lbbox = label  # （52,52,3,5+mix+class）,（26,26,3,5+mix+class）,（13,13,3,
        # 5+mix+class）
        sbboxes, mbboxes, lbboxes = bboxes_xywh  # （150,4）,(150,4),(150,4)
        return to_tensor(label_sbbox), to_tensor(label_mbbox), to_tensor(label_lbbox), to_tensor(sbboxes), to_tensor(
            mbboxes), to_tensor(lbboxes), torch.cat(target, dim=0)

    def __len__(self):
        return self.num_samples


if __name__ == '__main__':
    fog = dataset_fog('train')
    batch_size = cfg.TRAIN.BATCH_SIZE
    fog_loader = DataLoader(fog, batch_size=batch_size, collate_fn=fog.collate_fn)
    for loader in fog_loader:
        for i in range(len(loader)):
            print(loader[i].shape)
