...
def image_preprocess(image, target_size, gt_boxes=None):
    # 返回:image(归一化，RGB,hwc),box(num_gt,5(x1,y1,x2,y2,label)))
    # 将图片放缩成目标尺寸，并填充0
...
def random_crop_fog(image, foggy_image, bboxes):
    if random.random() < 0.5:
        h, w, _ = image.shape
        ...
        bboxes[:, [0, 2]] = bboxes[:, [0, 2]] - crop_xmin
        bboxes[:, [1, 3]] = bboxes[:, [1, 3]] - crop_ymin
    return image, foggy_image, bboxes
...
def random_horizontal_flip_fog(image, foggy_image, bboxes):
    if random.random() < 0.5:
        _, w, _ = image.shape
        image = image[:, ::-1, :]
...
def random_translate_fog(image, foggy_image, bboxes):
    if random.random() < 0.5:
        h, w, _ = image.shape
        max_bbox = np.concatenate([np.min(bboxes[:, 0:2], axis=0), np.max(bboxes[:, 2:4], axis=0)],
                                  axis=-1)
